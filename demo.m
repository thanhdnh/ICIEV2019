...
for l=1:length(Masks)
    for k=1:length(Inputs)
       ...
       result = uint8(inpaintingmixed(Input2));
       
       imwrite(result, strcat(path, '\proposed\', mfname, '_', ifname, '.jpg'));
       psnrs(i)=psnr(result, Input);
       ssims(i)=ssim(result, Input);
       fnames(i)=strcat(mfname,'_',ifname);
       i=i+1;
    end
end
exporter =[fnames psnrs ssims];
xlswrite(strcat(path,'\proposed\proposedmixed.xlsx'),exporter);